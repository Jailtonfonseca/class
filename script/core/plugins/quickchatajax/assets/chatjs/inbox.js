...
function chatWith(chatid,userid,fullname,userimage,userstatus,postid) {
    ...
    if (!$(this).hasClass('active')) {
        ...
        var userImage = '<img src="' + userimage + '" class="avatar-image is-loaded bg-theme" width="100%" alt="' + fullname + '">';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var profile_tpl = '<div class="">\n' +
                ...
                '                            <img class="thumb-lg img-circle" src="'+image+'" alt="'+name+'"></a>\n' +
            ...
            if(result.data) {
                $.each(result.data, function (i, item) {
                    ...
                    tpl += '<li class="start_wchat person chatboxhead" id="contact_' + item.chatid + '" ' +
                        ...
                        '<a href="#">' +
                        '<span class="userimage profile-picture min-profile-picture"><img src="' + item.userimage + '" class="avatar-image is-loaded bg-theme" width="100%" alt="' + item.fullname + '"></span>' +
                        '<span>' +
...
$('.wchat-chat-msgs').scroll(function(){
    ...
    if ($('.wchat-chat-msgs ').scrollTop() == 0){
        ...
        if($("#chatbox_"+chatid+" .pagenum:first").val() != $("#chatbox_"+chatid+" .total-page").val()) {
            ...
            get_all_msg(QUICKCHAT_AJAXURL+"?page="+pagenum+"&action=get_all_msg&client="+userid+"&postid="+postid);
