    ...
    public function update(Request $request)
    {
        ...
        if ($request->has('logo_setting')) {
            $validator = Validator::make($request->all(), [
                ...
                'banner_image' => 'nullable|mimes:jpg,jpeg',
            ...
            if ($request->has('banner_image') && $request->file('banner_image') != null) {
                $banner_image = image_upload($request->file('banner_image'), 'storage/banner/', '1028', null, config('settings.banner_image'));
                $requestData['banner_image'] = $banner_image;
            } else {
                $requestData['banner_image'] = config('settings.banner_image');
            }
         ...
         * Post Settings
         **/
        if ($request->has('post_setting')) {
            $validator = Validator::make($request->all(), [
                'max_image_upload' => ['required', 'integer', 'min:1', 'max:10'],
            ]);
            $errors = [];
            if ($validator->fails()) {
                foreach ($validator->errors()->all() as $error) {
            ...
            $requestData = $request->except('post_setting');
            foreach ($requestData as $key => $value) {
                Option::updateOptions($key, $value);
            }
         ...
         * Map Settings
         **/
        if ($request->has('map_settings')) {

            $requestData = $request->except('map_settings');
            foreach ($requestData as $key => $value) {
                Option::updateOptions($key, $value);
            }
         ...
         **/
        if ($request->has('live_location_settings')) {

            $requestData = $request->except('live_location_settings');
            foreach ($requestData as $key => $value) {
                Option::updateOptions($key, $value);
            }
