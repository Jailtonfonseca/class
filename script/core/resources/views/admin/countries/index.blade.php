...
@section('title', ___('Countries'))
@section('header_buttons')
    <a href="#" data-url="{{ route('admin.countries.create') }}" data-toggle="slidePanel" class="btn btn-primary ms-2"><i class="icon-feather-plus me-2"></i> {{ ___('Add New') }}</a>
@endsection
@section('content')
    ...
    <div class="quick-card card">
        <div class="card-body">
            <div class="dataTables_wrapper">
                <table class="table table-striped" id="ajax_datatable" data-jsonfile="{{ route('admin.countries.index') }}" data-order-dir="asc">
                    <thead>
                    <tr>
                        <th data-priority="1">{{ ___('ID') }}</th>
                        <th>{{ ___('Code') }}</th>
                        <th>{{ ___('country name') }}</th>
                        <th>{{ ___('status') }}</th>
    ...
    <div class="site-action">
        <div class="site-action-buttons">
            <button type="button" id="quick-delete-button" data-action="{{ route('admin.countries.delete') }}"
        ...
        <button type="button" class="front-icon btn btn-primary btn-floating"
                data-url="{{ route('admin.countries.create') }}" data-toggle="slidePanel">
