        ...
        <div class="quick-card card">
            ...
            <div class="card-body">
                <div class="row g-3 mb-2">
                    ...
                    <div class="col-lg-6">
                        <label class="form-label" for="home_page">{{ ___('Home Page Style') }} *</label>
                        <select name="home_page" id="home_page" class="form-control">
                            <option value="home_image" @selected(@$settings->home_page == "home_image")>{{ ___('Home with Image') }}</option>
                            <option value="home_map" @selected(@$settings->home_page == "home_map")>{{ ___('Home with Map') }}</option>
                        </select>
                    </div>
                    <div class="col-lg-6">
                        <label class="form-label" for="home_map_marker_limit">{{ ___('Home Map Marker Limit') }}</label>
                        <input type="text" id="home_map_marker_limit" name="home_map_marker_limit" class="form-control" value="{{ @$settings->home_map_marker_limit }}">
                    </div>
                    <div class="col-lg-6">
                        <label class="form-label" for="listing_view">{{ ___('Search Listing Default view') }} *</label>
                        <select name="listing_view" id="listing_view" class="form-control">
                            <option value="list" @selected(@$settings->listing_view == "list")>{{ ___('List View') }}</option>
                            <option value="grid" @selected(@$settings->listing_view == "grid")>{{ ___('Grid View') }}</option>
                        </select>
                    </div>
                    <div class="col-lg-12">
